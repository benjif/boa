...
vk::ImageView boa::Renderer::create_image_view(vk::Image image, vk::Format format, vk::ImageAspectFlags aspect_flags) {
...
void boa::Renderer::create_swapchain() {
    ...
    // TODO supported formats like last time
    m_depth_format = vk::Format::eD32Sfloat;

    vk::ImageCreateInfo img_create_info
        = image_create_info(m_depth_format, vk::ImageUsageFlagBits::eDepthStencilAttachment, depth_image_extent);
    ...
    vmaCreateImage(
        m_allocator,
        reinterpret_cast<VkImageCreateInfo *>(&img_create_info),
        &img_alloc_info,
    ...
    try {
        m_depth_image_view = m_device.get().createImageView(img_view_create_info);
    } catch (const vk::SystemError &err) {
